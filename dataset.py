...
def load_wav(full_path, sampling_rate): 
...
mel_basis, hann_window = dict(), dict()
...
def mel_spectrogram(y, n_fft, num_mels, sampling_rate, hop_size, win_size, fmin, fmax, center=False): 
  ...
  spec = torch.matmul(mel_basis[str(fmax)+'_'+str(y.device)], spec.permute(2, 1, 0)) 
  spec = spectral_normalize(spec)
  return spec 

class mel_dataset(data.Dataset): 
  def __init__(self, training_files, segment_size, n_fft, num_mels, 
               hop_size, win_size, sampling_rate, fmin, fmax, split=True, 
               shuffle=True, n_cache_reuse=1, device=None, fmax_loss=None, 
  ...
  def __getitem__(self, index): 
    ...
    if self._cache_ref_count == 0: 
      ...
      if sampling_rate != self.sampling_rate: 
        raise ValueError('{} SR does not match target {} SR'.format(sampling_rate, self.sampling_rate))
    ...
    if not self.fine_tuning: 
      ...
      mel = np.load(os.path.join(self.base_mels_path, os.path.splittext(os.path.split(filename)[-1])[0] + '.npy'))
    ...
    mel_loss = mel_spectrogram(audio, self.n_fft, self.num_mels, self.sampling_rate, 
